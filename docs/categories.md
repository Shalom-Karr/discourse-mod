...
Each method calls `super` first — if the base Discourse permission allows it, the plugin doesn't interfere. The plugin only adds permissions, never removes them.
